...
export default function Testimonials() {
  const slider = [
    <Testimonial name="Harry Wilson" title="Property Owner" photo={Ellipse4} />,
  ...
  return (
    <aside className="bg-red-50 py-8 md:py-2 ">
      <div className="lg:flex lg:flex-row-reverse md:max-w-xl lg:max-w-5xl mx-auto">
        <div>
          <img src={cover} alt="" className="md:max-w-xl md:mx-auto" />
        </div>
        <div className="">
          {slider[sliderIndex]}
            ...
            {dots.map((dot, index) =>
              index == sliderIndex ? (
                <div
                  ...
                  onClick={() => handleSliderDots(index)}
                >
                  {dot}
                </div>
              ) : (
